...
size_t FindSubstringSse42(std::string_view haystack, std::string_view needle) {
    ...
    constexpr size_t kChunkWidth = 16;
    const size_t haystackLen = haystack.size();
    const size_t needleLen = needle.size();
    const __m128i firstByte = _mm_set1_epi8(needle[0]);

    size_t chunkStart = 0;
    for (; chunkStart + kChunkWidth <= haystackLen; chunkStart += kChunkWidth) {
        __m128i chunk =
            _mm_loadu_si128(reinterpret_cast<const __m128i*>(haystack.data() + chunkStart));
        __m128i eq = _mm_cmpeq_epi8(chunk, firstByte);
        unsigned int mask = static_cast<unsigned int>(_mm_movemask_epi8(eq));
        while (mask != 0) {
